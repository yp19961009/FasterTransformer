...
private:
    ...
    ZipformerAttentionLayer<T>* longformer_attn_layer_;

    std::vector<ZipformerLayerWeight<T>> weights_;
...
public:
    ZipformerEncoder(size_t           layers_num,
                      size_t           in_dim,
                      size_t           head_num,
    ...
    std::vector<ZipformerLayerWeight<T>>* getWeightsPtr();

private:
    size_t getInitCubStorage(const int seq_len);
    void   initZipformerIdx(const T* global_attn_mask, const int seq_len, const int batch_size);
    void   allocateBuffer();
    void   freeBuffer();
